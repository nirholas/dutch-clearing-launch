...
/**
 * @title DeployDutchClearingLaunch
 * @notice Deploys DutchClearingLaunch deterministically to any chain with a Uniswap v4 `PoolManager`.
 *
 ...
 */
contract DeployDutchClearingLaunch is Script {
    ...
    function run() external {
        ...
        bytes memory creationCode = type(DutchClearingLaunchHook).creationCode;
        ...
        console2.log("DutchClearingLaunch ", predicted);
        ...
        vm.startBroadcast();
        DutchClearingLaunchHook hook = new DutchClearingLaunchHook{salt: salt}(manager);
        vm.stopBroadcast();
        ...
        require(
            keccak256(bytes(IHookMetadata(address(hook)).hookName())) == keccak256(bytes("DutchClearingLaunch")),
