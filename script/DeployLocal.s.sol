...
contract DeployLocal is Script {
    ...
    function run() external {
        ...
        (address predicted, bytes32 salt) = HookMiner.find(
            CREATE2_DEPLOYER, FLAGS, type(DutchClearingLaunchHook).creationCode, abi.encode(IPoolManager(address(manager)))
        );
        hook = new DutchClearingLaunchHook{salt: salt}(IPoolManager(address(manager)));
    ...
    function _openPool() private {
        ...
        hook.configure(key, DutchClearingLaunchHook.Config({startPriceX96: uint160(4 * Q96), floorPriceX96: uint160(Q96 / 2), startTime: uint64(block.timestamp), duration: 3600, buyIsZeroForOne: false}));
