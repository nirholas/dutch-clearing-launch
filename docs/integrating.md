# Integrating DutchClearingLaunch

## From Solidity

```solidity
import {DutchClearingLaunchHook} from "dutch-clearing-launch/src/hooks/DutchClearingLaunchHook.sol";
...
hook.configure(key, DutchClearingLaunchHook.Config({ /* see the README */ }));
...
const hook = getHook("dutch-clearing-launch");
const address = hookAddress("dutch-clearing-launch", 8453);              // Base
...
```solidity
IHookMetadata(hookAddress).hookName();   // "DutchClearingLaunch"
...
More at https://dutch-clearing-launch.pages.dev.
